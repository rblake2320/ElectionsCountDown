...
async function auditElectionData() {
  ...
  try {
    ...
    console.log(
      `📊 Found ${dbElections.length} elections in database from today forward`,
    );

    // Check for today's elections specifically
    const today = new Date().toISOString().split("T")[0];
    const todayElections = dbElections.filter(
    ...
    if (todayElections.length === 0) {
      ...
      todayElections.forEach((election) => {
        console.log(
          `  - ${election.title} (${election.state}, ${election.level})`,
        );
      });
    }
    ...
    console.log(
      `🗳️ Special elections in next 6 months: ${specialElectionQuery.rows[0].count}`,
    );
    ...
    if (todayElections.length === 0) {
      console.log(
        "1. ⚠️  CRITICAL: No elections found for today - verify with official sources",
      );
    }
...
async function discoverMissingElections() {
  ...
  electionPatterns.forEach((pattern) => {
    if (pattern.months.includes(currentMonth)) {
      console.log(
        `🎯 Expected: ${pattern.description} (${pattern.type}, ${pattern.level})`,
      );
    }
  });
