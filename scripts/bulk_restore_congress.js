...
async function bulkRestoreCongressMembers() {
  try {
    ...
    const membersData = JSON.parse(
      fs.readFileSync('./attached_assets/congress_members_complete.json', 'utf8')
    );
    ...
    for (let i = 0; i < membersData.length; i += batchSize) {
      ...
      const insertData = batch.map((member, index) => ({
        bioguideId:
          member.bioguide_id ||
          `${member.full_name.replace(/\s+/g, '_').toUpperCase()}_${i}_${index}`,
      ...
      console.log(
        `Batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(membersData.length / batchSize)} completed. Total: ${totalInserted}`
      );
    }
