...
export function PollingTrendChart({
  electionId,
  candidates,
  pollingData,
  className,
  ...
  const [selectedCandidates, setSelectedCandidates] = useState<number[]>(candidates.map(c => c.id));

  const filteredData = useMemo(() => {
    const cutoffDate = subDays(new Date(), parseInt(timeRange));
    return pollingData.filter(
      point => parseISO(point.date) >= cutoffDate && selectedCandidates.includes(point.candidateId)
    );
    ...
    return (
      <Card className={className}>
        <CardHeader>
          ...
          <CardDescription>No polling data available for this election</CardDescription>
        </CardHeader>
