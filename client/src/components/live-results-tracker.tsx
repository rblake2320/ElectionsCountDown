...
export function LiveResultsTracker({
  election,
  isLive = false,
}: LiveResultsTrackerProps) {
  ...
  const { candidates, hasResults, winner, totalVotes, reportingPercent } =
    liveResults;
      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              <RefreshCw
                className={cn("w-3 h-3 mr-1", autoRefresh && "animate-spin")}
              />
              Auto
            </Button>
      ...
      <CardContent className="space-y-4">
        {!hasResults ? (
          <div className="text-center py-8 space-y-2">
            ...
            <p className="text-sm text-text-muted">
              Results will appear when polls close
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {sortedCandidates.map((candidate, index) => {
              const votePercent =
                totalVotes > 0
                  ? ((candidate.votesReceived || 0) / totalVotes) * 100
                  : 0;
          ...
          <div className="mt-6 p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
            <div className="flex items-center space-x-2 text-green-700">
              ...
              <div>
                ...
                <p className="text-sm">
                  {winner.name} ({winner.party}) wins with{" "}
                  {winner.votePercentage}% of the vote
                </p>
              </div>
            </div>
          </div>
