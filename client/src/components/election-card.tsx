...
export function ElectionCard({ election, viewMode = 'grid' }: ElectionCardProps) {
  ...
  const isInWatchlist =
    Array.isArray(watchlist) && watchlist.some((item: any) => item.electionId === election.id);
  ...
  const urgencyColor =
    daysUntil <= 7 ? 'bg-brand-error' : daysUntil <= 30 ? 'bg-brand-accent' : 'bg-brand-primary';
  ...
  return (
    ...
    >
      ...
      <header className={cn('space-y-2 flex-1', viewMode === 'grid' ? 'mt-8' : '')}>
        <h2
          className={cn(
        ...
        <div className="space-y-1">
          <p className="text-sm text-muted-foreground">{election.subtitle || 'General Election'}</p>
      ...
      {candidatesLoading ? (
        ...
        <section className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            {candidates.length > 4 && (
              <span className="text-xs text-gray-500 dark:text-gray-400">Showing top 4</span>
            )}
          </div>
            ...
            {candidates.slice(0, 4).map((candidate: any) => (
              <div
                ...
                className={cn(
                  ...
                  candidate.isWinner ? 'border-green-500/50 bg-green-500/10' : 'border-border/50'
                )}
              >
                <div className="space-y-1">
                              ...
                              <TooltipTrigger asChild>
                                <div
                                  className={`w-2 h-2 rounded-full ${
                                    candidate.lastPollingUpdate ? 'bg-green-500' : 'bg-red-500'
                                  }`}
                                />
                              </TooltipTrigger>
                              <TooltipContent>
                          ...
                          <TooltipTrigger asChild>
                            <div
                              className={`w-1.5 h-1.5 rounded-full ${
                                candidate.dataAuthenticity.dataQuality === 'excellent'
                                  ? 'bg-green-500'
                                  : candidate.dataAuthenticity.dataQuality === 'good'
                                    ? 'bg-blue-500'
                                    : candidate.dataAuthenticity.dataQuality === 'fair'
                                      ? 'bg-yellow-500'
                                      : 'bg-gray-500'
                              }`}
                            />
      ...
      <footer className="mt-auto flex justify-between items-center pt-4 border-t border-border/20">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {candidates && Array.isArray(candidates) && candidates.length >= 2 && (
            <CandidateComparisonWizard election={election} candidates={candidates as Candidate[]} />
          )}
