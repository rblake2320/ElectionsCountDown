...
export function EnhancedPolicyComparison({
  candidateDetails,
  selectedCategories,
}: EnhancedPolicyComparisonProps) {
  ...
  const getPositionStrength = (
    ...
    if (
      position.toLowerCase().includes("not available") ||
      position.toLowerCase().includes("not specified")
    ) {
      return "weak";
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {candidateDetails.map((candidate, index) => {
          ...
          return (
            <Card
              key={candidate.id}
              className={`${colors.light} ${colors.border} border-2 transition-all hover:shadow-lg`}
            >
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-4 h-4 rounded-full ${colors.primary}`}
                    ></div>
                    ...
                    {[
                      {
                        key: "propublica",
                        label: "ProPublica",
                        available: candidate.dataSourceAvailability?.propublica,
                      },
                      {
                        key: "fec",
                        label: "FEC",
                        available: candidate.dataSourceAvailability?.fec,
                      },
                      {
                        key: "voteSmart",
                        label: "VoteSmart",
                        available: candidate.dataSourceAvailability?.voteSmart,
                      },
                      {
                        key: "openStates",
                        label: "OpenStates",
                        available: candidate.dataSourceAvailability?.openStates,
                      },
                    ].map((source) => (
                      <div key={source.key} className="flex items-center gap-1">
                        ...
                        <span
                          className={
                            source.available ? "text-gray-700" : "text-gray-400"
                          }
                        >
                          {source.label}
                        </span>
        ...
        {selectedCategories.map((category) => (
          <Card key={category} className="overflow-hidden shadow-sm border-2">
            <CardHeader className="bg-gradient-to-r from-gray-50 to-gray-100">
              ...
              <CardDescription>
                Comparative analysis from official voting records and verified
                sources
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <div className="divide-y divide-gray-100">
                {candidateDetails.map((candidate, index) => {
                  const colors =
                    candidateColorSchemes[index % candidateColorSchemes.length];
                  const policy = candidate.policies?.find(
                    (p) => p.category === category,
                  );
                  const position =
                    policy?.position ||
                    "Position not available from current sources";
                  const strength = getPositionStrength(position);

                  return (
                    <div
                      key={candidate.id}
                      className={`p-6 ${colors.light} border-l-4 ${colors.border.replace("border-", "border-l-")}`}
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-3 h-3 rounded-full ${colors.primary}`}
                          ></div>
                          <h4
                      ...
                      <div className="space-y-3">
                        ...
                        {policy?.details && (
                          <div className="bg-white/60 p-3 rounded border">
                            <h6 className="font-medium text-gray-800 mb-1 text-sm">
                              Additional Context
                            </h6>
              ...
              <div className="text-2xl font-bold text-gray-900 mb-1">
                {Math.round(
                  candidateDetails.reduce(
                    (acc, c) => acc + getDataSourceScore(c),
                    0,
                  ) / candidateDetails.length,
                )}
                %
