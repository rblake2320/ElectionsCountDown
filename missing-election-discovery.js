...
async function discoverMissingElections() {
  ...
  const potentialMissingElections = [
    ...
    },
    {
      ...
      type: "Special",
      description:
        "Fire district, water district, or other special purpose elections",
    },
    {
...
async function verifyElectionCompleteness() {
  ...
  try {
    ...
    if (statesWithFewElections.length > 0) {
      gaps.push(
        `Limited coverage in: ${statesWithFewElections.slice(0, 5).join(", ")}`,
      );
    }
...
async function main() {
  ...
  console.log(
    `Total elections in database: ${verification?.totalElections || "Unknown"}`,
  );
