...
export default function () {
  ...
  let response = http.get(
    `${BASE_URL}/api/elections?state=${state}&type=${type}`,
  );
  ...
  if (response.status === 200) {
    ...
    if (elections.length > 0) {
      ...
      // Get candidates
      response = http.get(
        `${BASE_URL}/api/elections/${randomElection.id}/candidates`,
      );
      check(response, {
...
export function campaignAnalytics() {
  ...
  check(response, {
    "campaign analytics accessible": (r) =>
      r.status === 200 || r.status === 401, // 401 expected for test keys
  });
