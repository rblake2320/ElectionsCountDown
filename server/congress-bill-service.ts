...
export class CongressBillService {
    ...
    try {
      ...
      while (true) {
        ...
        const data = await this.makeRequest(
          `/member/congress/${congressNumber}`,
          {
            offset: offset.toString(),
            limit: limit.toString(),
          },
        ...
        const members = data.members.map((member: any) => ({
          ...
          state: member.state,
          chamber:
            member.terms?.item?.[0]?.chamber?.replace(
      ...
      console.log(
        `${congressNumber}th Congress: ${allMembers.length} members fetched`,
      );
      return allMembers;
    } catch (error) {
      console.error(
        `Error fetching ${congressNumber}th Congress members:`,
        error,
      );
      return [];
    }
  ...
  async fetchAllMembers(): Promise<CongressMember[]> {
    try {
      console.log(
        "Fetching current serving members from 119th Congress (current Congress)...",
      );
  ...
  private async fetchCurrentHouseMembers(): Promise<CongressMember[]> {
    try {
      ...
      console.log(
        "House-specific endpoint not available, using general endpoint",
      );
      return [];
    }
  }
  ...
  private async fetchCurrentSenateMembers(): Promise<CongressMember[]> {
    try {
      ...
      console.log(
        "Senate-specific endpoint not available, using general endpoint",
      );
      return [];
    }
  }
  ...
  async fetchSenateCommunications(
    congress: string = "118",
  ): Promise<SenateCommunication[]> {
    try {
...
export function getCongressBillService(): CongressBillService | null {
  const apiKey =
    process.env.CONGRESS_API_KEY || "cc9mECbK6VKcz0ChKLUo85xZr6kySbIM9kTiy45M";
  if (!apiKey) {
