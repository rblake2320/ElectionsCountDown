...
export class CandidatePositionAPI {
  ...
  async getPositionsForCandidate(candidateId: number): Promise<PositionData[]> {
    ...
    for (const category of this.POLICY_CATEGORIES) {
      const positionData = await this.analyzePositionForCategory(
        candidate,
        category,
      );
      if (positionData) {
  ...
  private async analyzePositionForCategory(
    ...
    if (candidate.congressMember) {
      const congressionalPosition = await this.getCongressionalPosition(
        candidate,
        category,
      );
    ...
    const partyPosition = await this.getPartyBasedPosition(
      candidate.candidate,
      category,
    );
    if (partyPosition) {
  ...
  private getTypicalPartyPositions(party: string, category: string) {
    ...
    const republicanPositions = {
      ...
      Taxes:
        "Advocates for lower taxes, simplified tax code, and pro-business tax policies",
    };
