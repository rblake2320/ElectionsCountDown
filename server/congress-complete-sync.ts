...
export class CongressCompleteSyncService {
  ...
  async fetchAllCurrentMembers() {
    ...
    console.log(
      `Expected total: ${totalExpected} members (435 House + 100 Senate + 6 delegates)`,
    );
    ...
    for (const delegation of US_CONGRESSIONAL_DELEGATIONS) {
      try {
        ...
        const stateMembers = await this.congressService.fetchMembersByState(
          delegation.state,
        );
        ...
        if (stateMembers && stateMembers.length > 0) {
          ...
          console.log(
            `No members found for ${delegation.state} - using general API as fallback`,
          );
        }
    ...
    console.log(
      `Fetched ${uniqueMembers.length} unique members from state-by-state approach`,
    );
    ...
    if (uniqueMembers.length < 500) {
      ...
      console.log(
        "State-by-state approach incomplete, trying general endpoint as supplement...",
      );
      ...
      try {
        ...
        console.log(
          `Combined approach yielded ${finalMembers.length} total members`,
        );
        return finalMembers;
      } catch (error) {
  ...
  validateMemberCounts(members: any[]) {
    ...
    for (const delegation of US_CONGRESSIONAL_DELEGATIONS) {
      const actual = membersByState[delegation.state] || {
        house: 0,
        senate: 0,
      };
