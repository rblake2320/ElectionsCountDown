...
export class FirecrawlService {
  ...
  async scrapeUrl(
    url: string,
    includeHtml = false,
  ): Promise<ScrapedElectionData | null> {
    try {
  ...
  async scrapeMultiple(urls: string[]): Promise<ScrapedElectionData[]> {
    ...
    return results
      .filter(
        (result): result is PromiseFulfilledResult<ScrapedElectionData> =>
   ...
   */
  async scrapeStateElectionSites(
    state: string,
  ): Promise<ScrapedElectionData[]> {
  ...
  async scrapeCandidateInfo(
    ...
    try {
      ...
      console.error(
        `Error scraping candidate info for ${candidateName}:`,
        error,
      );
      return null;
    }
  ...
  private extractCandidateInfo(
    ...
    try {
      ...
      const partyMatch = content.match(
        new RegExp(
          `${candidateName}.*?(Republican|Democratic|Independent|Green|Libertarian)`,
          "i",
        ),
      );
      const officeMatch = content.match(
        new RegExp(`(Senator|Representative|Governor|Mayor|Congress)`, "i"),
      );
   ...
   */
  private extractBiography(
    content: string,
    candidateName: string,
  ): string | undefined {
    const sentences = content.split(/[.!?]+/);
    const bioSentences = sentences.filter(
      (sentence) =>
...
export function getFirecrawlService(): FirecrawlService | null {
  if (!process.env.FIRECRAWL_API_KEY) {
    console.warn(
      "FIRECRAWL_API_KEY not found. Web scraping will not be available.",
    );
    return null;
  }
 ...
 */
export async function scrapeOfficialElectionData(
  state: string,
...
export async function enrichCandidateWithWebData(
  candidateName: string,
  office: string,
  state: string,
): Promise<CandidateInfo | null> {
...
export async function monitorElectionNews(): Promise<ScrapedElectionData[]> {
  ...
  return await service.scrapeElectionNews([
    "election",
    "voting",
    "results",
    "ballot",
