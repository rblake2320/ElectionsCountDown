...
export class CongressImportService {
  async importFromCompleteDataset(): Promise<{
    count: number;
    breakdown: any;
  }> {
    try {
      ...
      const filePath = path.join(
        process.cwd(),
        "attached_assets",
        "congress_members_complete.json",
      );
      ...
      const supplementPath = path.join(
        process.cwd(),
        "attached_assets",
        "missing_members_supplement.json",
      );
      const supplementContent = fs.readFileSync(supplementPath, "utf8");
      const missingMembers: CongressMemberImport[] =
        JSON.parse(supplementContent);

      // Combine both datasets
      const allMembers = [...membersData, ...missingMembers];

      console.log(
        `Found ${membersData.length} members in main dataset + ${missingMembers.length} missing members = ${allMembers.length} total`,
      );
      ...
      console.log(
        `After cleaning duplicates and fixing issues: ${cleanedData.length} members`,
      );

      // Transform and insert data
      const insertData = cleanedData.map((member, index) => ({
        ...
        district:
          member.district === "Senate" || member.district === "At-large"
            ? null
            : member.district,
  ...
  private removeDuplicatesAndFix(
    allMembers: CongressMemberImport[],
  ): CongressMemberImport[] {
    ...
    console.log(
      `Processing ${allMembers.length} members without aggressive deduplication`,
    );
    return allMembers;
  }

  private cleanAndDeduplicateMembers(
    membersData: CongressMemberImport[],
  ): CongressMemberImport[] {
    ...
    for (const [state, senators] of senateByState) {
      if (senators.length <= 2) {
        ...
        console.log(
          `State ${state} has ${senators.length} senators, selecting 2 most current`,
        );

        // Filter out known former members first
        const activeSenators = senators.filter(
          (s) =>
    ...
    for (const [district, representatives] of houseByDistrict) {
      if (representatives.length === 1) {
        ...
        console.log(
          `District ${district} has ${representatives.length} representatives:`,
          representatives.map((r) => r.full_name),
        );
        ...
        const selectedRep =
          activeReps.length > 0 ? activeReps[0] : representatives[0];
    ...
    // Validate counts
    const houseCount = cleanedMembers.filter(
      (m) => m.chamber === "House",
    ).length;
    const senateCount = cleanedMembers.filter(
      (m) => m.chamber === "Senate",
    ).length;
    const stateCount = new Set(
      cleanedMembers.filter((m) => m.chamber === "Senate").map((m) => m.state),
    ).size;

    console.log(
      `Cleaned data: ${houseCount} House + ${senateCount} Senate = ${cleanedMembers.length} total`,
    );
  ...
  async validateImport(): Promise<any> {
    try {
      ...
      // Count by party
      const partyBreakdown = totalMembers.reduce(
        (acc, member) => {
