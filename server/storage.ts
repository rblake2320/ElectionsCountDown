...
export class DatabaseStorage implements IStorage {
  async getElections(filters?: ElectionFilters): Promise<Election[]> {
    ...
    const results = await electionQuery.where(and(...allConditions)).orderBy(elections.date);
  ...
  async getElectionStats(): Promise<{
    ...
    const allElections = await db.select().from(elections).where(eq(elections.isActive, true));
    ...
    try {
      ...
      for (const election of googleElections) {
        ...
        const existing = await db
          .select()
          .from(elections)
          .where(and(eq(elections.title, election.title), eq(elections.date, election.date)))
          .limit(1);
  ...
  async getVoterInfo(address: string): Promise<any> {
    ...
    try {
      ...
      const googleElections = await civicService.fetchElections();
      const upcomingGoogleElection = googleElections.find(election => election.date > new Date());
  ...
  async addToWatchlist(userId: string, electionId: number): Promise<WatchlistItem> {
    try {
      ...
      const existing = await db
        .select()
        .from(watchlist)
        .where(and(eq(watchlist.userId, userId), eq(watchlist.electionId, electionId)));
  ...
  async getElectionCycle(slug: string): Promise<any> {
    try {
      ...
      const [cycle] = await db.select().from(electionCycles).where(eq(electionCycles.slug, slug));
  ...
  async authenticateCandidate(email: string, password: string): Promise<CandidateAccount | null> {
    try {
      const [account] = await db
        ...
        .where(and(eq(candidateAccounts.email, email), eq(candidateAccounts.isActive, true)))
        .limit(1);
  ...
  async recordDataSource(source: InsertCandidateDataSource): Promise<CandidateDataSource> {
    try {
      const [newSource] = await db.insert(candidateDataSources).values(source).returning();
  ...
  async getCampaignSubscription(campaignId: number): Promise<any> {
    ...
    return {
      ...
      features:
        SUBSCRIPTION_TIERS[campaign.subscriptionTier as keyof typeof SUBSCRIPTION_TIERS]
          ?.features || [],
    };
  }
}
