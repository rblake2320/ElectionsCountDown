...
export class BotPreventionService {
    ...
    return {
      ...
      verificationLevel:
        riskScore < 0.3
          ...
          : VERIFICATION_LEVELS.UNVERIFIED,
      actionRecommended:
        riskScore > 0.5 ? "require_additional_verification" : "proceed",
      reasons: risks,
    };
  }
  ...
  async analyzeBehavior(
    ...
    if (clickPatterns.length > 0) {
      const avgTimeBetweenClicks =
        this.calculateAverageTimeBetweenClicks(clickPatterns);
  ...
  async performContinuousVerification(
    ...
    return {
      ...
      actionRecommended:
        riskScore > 0.7 ? "require_reverification" : "continue_monitoring",
      reasons: risks,
    };
  }
  ...
  async getUserVerificationStatus(userId: number): Promise<any> {
    ...
    return {
      ...verification,
      isVerifiedForAnalytics:
        verification.verificationLevel >= VERIFICATION_LEVELS.PHONE_VERIFIED,
    };
  }
  ...
  async getCampaignAnalytics(electionId: number): Promise<any> {
    ...
    return {
      ...
      confidence_level:
        verifiedHumans / (totalInterest || 1) > 0.95 ? "Very High" : "High",
