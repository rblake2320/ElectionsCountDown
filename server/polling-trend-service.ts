...
export class PollingTrendService {
  ...
  private generateHistoricalData(
    ...
    const baseSupport =
      candidate.pollingSupport || this.getBaseSupportByParty(candidate.party);
    ...
    for (let i = days; i >= 0; i -= Math.floor(Math.random() * 5) + 3) {
      ...
      const support = Math.max(
        5,
        Math.min(65, baseSupport + variation + trendFactor),
      );
  ...
  async getTrendAnalysis(electionId: number): Promise<TrendAnalysis[]> {
    ...
    for (const candidate of electionCandidates) {
      ...
      const older = candidatePolling.slice(
        0,
        Math.min(3, candidatePolling.length - 3),
      );

      const recentAvg =
        recent.reduce((sum, p) => sum + p.support, 0) / recent.length;
      const olderAvg =
        older.length > 0
  ...
  async getPollingDataForElection(
    electionId: number,
    timeRange: string = "30",
  ): Promise<{
