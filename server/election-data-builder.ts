...
const REAL_2026_ELECTIONS: InsertElection[] = [
  ...
  },
  {
    ...
    description:
      "Election for Arizona Governor and state constitutional officers",
    pollsOpen: "6:00 AM MST",
    pollsClose: "7:00 PM MST",
    timezone: "MST",
    isActive: true,
...
export async function populateRealElectionData(): Promise<void> {
  try {
    ...
    console.log(
      `Added ${REAL_2026_ELECTIONS.length} real elections to database`,
    );
    ...
    if (perplexityService) {
      try {
        ...
        console.log(
          "AI found additional elections:",
          aiResults.substring(0, 500) + "...",
        );
      } catch (error) {
