...
export function registerCandidatePortalRoutes(app: Express) {
  ...
  app.use(
    ...
    candidateSecurityService.authenticateCandidate.bind(candidateSecurityService)
  );
    ...
    async (req, res) => {
      try {
        ...
        const position = await candidateManagementService.createPosition(candidateId, {
          ...
          detailedStatement:
            validation.sanitized.substring(validatedData.position.length).trim() ||
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const analytics = await candidateManagementService.getCandidateAnalytics(
          candidateId,
          timeframe
        );
