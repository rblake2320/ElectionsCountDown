export class PerplexityCongressService {
  ...
  async findMissingCongressMembers(currentMembers: any[]): Promise<any> {
    const houseCount = currentMembers.filter(
      (m) => m.chamber === "House",
    ).length;
    const senateCount = currentMembers.filter(
      (m) => m.chamber === "Senate",
    ).length;
    ...
    const messages = [
      {
        role: "system",
        content:
          "Verify congressional member information using current, official sources.",
      },
      {
        role: "user",
