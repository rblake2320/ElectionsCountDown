...
export class EventProcessingService extends EventEmitter {
  ...
  private processors: Map<string, (event: ElectionEvent) => Promise<void>> =
    new Map();
  ...
  private initializeProcessors(): void {
    ...
    this.processors.set(
      "candidate_change",
      this.processCandidateChange.bind(this),
    );
    this.processors.set(
      "precinct_report",
      this.processPrecinctReport.bind(this),
    );
    this.processors.set("district_call", this.processDistrictCall.bind(this));
    this.processors.set(
      "recount_declared",
      this.processRecountDeclaration.bind(this),
    );
  }
  ...
  private async checkDistrictCompletion(
    electionId: number,
    districtId: string,
  ): Promise<void> {
    ...
    const reportedPrecincts = precincts.filter(
      (p) => p.reportingComplete,
    ).length;
