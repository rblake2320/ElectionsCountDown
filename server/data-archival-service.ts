...
export class DataArchivalService {
  ...
  async archiveOldAnalytics(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Archived interaction logs older than ${ninetyDaysAgo.toISOString()}`,
      );
    } catch (error) {
  ...
  async archiveCompletedElections(): Promise<void> {
    ...
    try {
      ...
      const completedElections = await db
        ...
        .from(elections)
        .where(
          and(lt(elections.date, thirtyDaysAgo), eq(elections.isActive, false)),
        );
      ...
      if (completedElections.length > 0) {
        ...
        console.log(
          `Archived ${completedElections.length} completed elections`,
        );
      }
    } catch (error) {
