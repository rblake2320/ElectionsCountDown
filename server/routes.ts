...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/congressional-members/by-state/:state", async (req, res) => {
    try {
      ...
      console.error(
        `Error fetching members for state ${req.params.state}:`,
        error,
      );
      res
        .status(500)
        .json({ error: "Failed to fetch state congressional members" });
    }
  });
  ...
  app.get("/api/candidates/:id/positions", async (req, res) => {
    try {
      ...
      const positions =
        await candidatePositionAPI.getPositionsForCandidate(candidateId);
      res.json(positions);
    } catch (error) {
  ...
  app.get("/api/elections/:id/candidates", async (req, res) => {
    try {
      ...
      const { dataAuthenticityService } = await import(
  ...
  app.post("/api/scrape-election-data", async (req, res) => {
    try {
      ...
      const { scrapeOfficialElectionData } = await import(
        "./firecrawl-service"
      );
  ...
  app.post("/api/candidates/:id/enrich", async (req, res) => {
    try {
      ...
      const { enrichCandidateWithWebData } = await import(
        "./firecrawl-service"
      );
  ...
  app.get("/api/test-browser-automation", async (req, res) => {
    try {
      const { testBrowserCapabilities } = await import(
  ...
  app.post("/api/scrape-election-site", async (req, res) => {
    try {
      ...
      const { scrapeOfficialElectionSite } = await import(
        "./browser-automation-service"
      );
      ...
      res.json({
        ...
        hasScreenshot:
          scrapingResult.screenshots && scrapingResult.screenshots.length > 0,
        metadata: {
  ...
  app.post("/api/elections/:id/update-polling", async (req, res) => {
    try {
      ...
      const { getAuthenticPollingService } = await import(
        "./authentic-polling-service"
      );
      const pollingService = getAuthenticPollingService();
      const pollingData = await pollingService.getAuthenticPollingData(
        electionId,
        candidateNames,
      );
      ...
      res.json({
        ...
        updated: true,
        pollingData: {
          candidatesUpdated: pollingData.averages.filter(
            (a) => a.averagePercentage !== null,
          ).length,
  ...
  app.get("/api/elections/:id/polling-trends", async (req, res) => {
    try {
      ...
      const data = await pollingTrendService.getPollingDataForElection(
        electionId,
        timeRange,
      );
      res.json(data);
  ...
  app.post("/api/sync-elections", async (req, res) => {
    try {
      ...
      res.json({
        message: "Elections synced successfully from Google Civic API",
      });
    } catch (error) {
      res
        .status(500)
        .json({ error: "Failed to sync elections from Google Civic API" });
    }
  });
  ...
  app.get("/api/reverse-geocode", async (req, res) => {
    try {
      ...
      const location = await mapQuestService.reverseGeocode(
        latitude,
        longitude,
      );
  ...
  app.post("/api/cleanup-elections", async (req, res) => {
    try {
      ...
      res.json({
        ...
        status:
          removedCount === endedElections.length
            ? "cleanup_completed"
            : "partial_cleanup",
      });
    } catch (error) {
  ...
  app.get("/api/elections-by-location", async (req, res) => {
    try {
      ...
      if (!location) {
        return res
          .status(400)
          .json({ error: "Valid address or coordinates required" });
      }
      ...
      const localElections = stateElections.filter(
        (election) =>
          ...
          (election.location
            ...
            election.location
              ?.toLowerCase()
              .includes(location.county.toLowerCase()) ||
            election.subtitle
              ?.toLowerCase()
              .includes(location.city.toLowerCase())),
      );
      ...
      res.json({
        location,
        elections:
          localElections.length > 0
            ? localElections
            : stateElections.slice(0, 10),
  ...
  app.post("/api/congress/sync-all", async (req, res) => {
    try {
      console.log(
        "Starting complete Congress import from authenticated dataset...",
      );

      const { congressImportService } = await import(
        "./congress-import-service"
      );
  ...
  app.get("/api/members/search", async (req, res) => {
    try {
      ...
      console.log(
        "[ROUTES] Congressional search endpoint hit with term:",
        searchTerm,
      );
      if (!searchTerm) {
      ...
      console.log(
        `[ROUTES] Search for "${searchTerm}" returned ${members.length} results`,
      );
      res.json(members);
    } catch (error: any) {
  ...
  app.post("/api/elections/:id/results", async (req, res) => {
    try {
      ...
      const results = await storage.updateElectionResults(
        electionId,
        resultsData,
      );
      res.json(results);
...
Please provide detailed, current information about this specific election.`;
  ...
  app.post("/api/elections/:id/update-results", async (req, res) => {
    try {
      ...
      if (!electionId || !candidates) {
        return res
          .status(400)
          .json({ message: "Missing election ID or candidate results" });
      }
  ...
  app.post("/api/congress/find-missing", async (req, res) => {
    try {
      ...
      const result =
        await perplexityCongressService.findMissingCongressMembers(
          currentMembers,
        );
      res.json(result);
    } catch (error: any) {
  ...
  app.get("/api/election-cycles", async (req, res) => {
    ...
    try {
      ...
      const cycles = [
        ...new Set(elections.map((e) => new Date(e.date).getFullYear())),
      ]
        .sort((a, b) => a - b)
        .map((year) => ({
          id: year,
  ...
  app.get("/api/campaign/polling/:electionId", async (req, res) => {
    try {
      ...
      const polling = await storage.getCampaignPolling(
        campaign.id,
        electionId,
        dateRange,
      );
  ...
  app.post("/api/campaign/export", async (req, res) => {
    try {
      ...
      const exportData = await storage.purchaseDataExport(
        campaign.id,
        datasetType,
        format,
      );
  ...
  app.post("/api/admin/optimize/maintenance", async (req, res) => {
    try {
      const { databaseOptimizationService } = await import(
  ...
  app.post("/api/admin/backup/pre-archival", async (req, res) => {
    try {
      ...
      res.json({
        message: "Pre-archival backup created successfully",
        paths: backupPaths,
      });
    } catch (error: any) {
...
Please provide specific, factual information with sources where possible. Focus on verifiable policy positions and background information.`;
          ...
          try {
            ...
            if (!response.ok) {
              ...
              throw new Error(
                `Perplexity API error: ${response.statusText} - ${errorText}`,
              );
            }
            ...
            console.error(
              `Error fetching details for candidate ${candidate.name}:`,
              error,
            );
            ...
            try {
              ...
              console.error(
                `Error fetching RAG data for candidate ${candidate.id}:`,
                ragError,
              );
              return {
                id: candidate.id,
  ...
  app.get("/api/civic/compare", async (req, res) => {
    try {
      ...
      if (!candidateIds || !policyCategories) {
        return res
          .status(400)
          .json({ message: "Missing candidateIds or policyCategories" });
      }

      const ids = candidateIds.split(",");
      const categories = policyCategories.split(",");

      const comparison = await civicAggregatorService.comparePolicies(
        ids,
        categories,
      );
      res.json(comparison);
  ...
  app.get("/api/civic/ballot-info", async (req, res) => {
    try {
      ...
      const ballotInfo =
        await civicAggregatorService.fetchGoogleCivicData(address);
      res.json(
        ballotInfo || { address, message: "No ballot information available" },
      );
    } catch (error: any) {
  ...
  app.get("/api/civic/international", async (req, res) => {
    try {
      ...
      if (!candidateName || !country) {
        return res
          .status(400)
          .json({ message: "CandidateName and country are required" });
      }

      const internationalData =
        await civicAggregatorService.fetchInternationalData(
          candidateName,
          country,
        );
      res.json(
        internationalData || {
          candidateName,
          country,
          message: "No international data available",
        },
      );
    } catch (error: any) {
  ...
  app.post("/api/scrape/election", async (req, res) => {
    try {
      ...
      if (!scrapedData) {
        return res
          .status(404)
          .json({ error: "No election data found at the provided URL" });
      }
  ...
  app.get("/api/global/elections", async (req, res) => {
    try {
      ...
      const elections = await globalElectionService.fetchIDEAElections(
        country as string,
      );
      res.json(elections);
    } catch (error) {
  ...
  app.post("/api/global/ballot-info", async (req, res) => {
    try {
      ...
      const ballotInfo =
        await globalElectionService.getEnhancedBallotInfo(address);
      res.json(ballotInfo);
    } catch (error) {
  ...
  app.post("/api/events/ingest", async (req, res) => {
    try {
      ...
      const eventId = await eventProcessingService.ingestEvent(
        event,
        source || "api",
      );
  ...
  app.post("/api/crowdsource/report", async (req, res) => {
    try {
      ...
      const success =
        await globalElectionService.submitCrowdsourcedReport(report);
      res.json({ success });
    } catch (error) {
  ...
  const { addPercentageValidationEndpoint } = await import(
    "./percentage-validation-endpoint"
  );
  ...
  app.post("/api/sync/elections/all", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to sync elections", details: error.message });
    }
  });
  ...
  app.get("/api/sync/status", async (req, res) => {
    try {
      ...
      res.json({
        ...
        lastSync,
        recommendation:
          stats.total < 601 ? "Run full election sync" : "Count maintained",
      });
    } catch (error) {
  ...
  app.post("/api/setup/michigan-primary", async (req, res) => {
    try {
      const { addMichiganPrimaryWithCandidates } = await import(
        "./add-michigan-election"
      );
...
function parsecandidateAnalysis(analysis: string, candidate: any) {
  ...
  const backgroundSection =
    analysis.match(/BACKGROUND & EXPERIENCE:(.*?)(?=\d+\.|$)/s)?.[1] || "";
  const background =
    backgroundSection
      .split("\n")
  ...
  const endorsementSection =
    analysis.match(/endorsements?[:\s]+(.*?)(?=\n\n|$)/is)?.[1] || "";
  const endorsements =
    endorsementSection
      .match(
        /[A-Z][^.]*(?:Association|Union|Party|Group|Organization|Coalition)[^.]*(?:\.|$)/g,
      )
      ?.slice(0, 5) || [];

  // Extract funding information
  const fundingSection =
    analysis.match(/funding[:\s]+(.*?)(?=\n\n|$)/is)?.[1] || "";
